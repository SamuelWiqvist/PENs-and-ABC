...
# load data
y_obs = CSV.read("data/gandk/y_obs_ecdf.csv"; allowmissing=:auto)[:,2]
...
datasets = convert(Array{Float32,2},CSV.read("data/gandk/ABC_datasets_ecdf.csv", allowmissing=:auto))

# load training data for DNN
X_training = zeros(Float32, 100, 100000,10)
X_val = zeros(Float32,100, 100000)
X_test = zeros(Float32,100,100000,2)
...
for i in 1:10
    println(i)
    X_training[:,:,i] = convert(Array{Float32,2}, CSV.read("data/gandk/X_training_ecdf"*string(i)*".csv"; allowmissing=:auto))'
...
for i in 1:2
    println(i)
    X_test[:,:,i] = convert(Array{Float32,2}, CSV.read("data/gandk/X_test_ecdf"*string(i)*".csv"; allowmissing=:auto))'
    y_test[:,:,i] = convert(Array{Float32,2}, CSV.read("data/gandk/y_test"*string(i)*".csv"; allowmissing=:auto))'
end

X_val = convert(Array{Float32,2}, CSV.read("data/gandk/X_val_ecdf.csv"; allowmissing=:auto))'
y_val = convert(Array{Float32,2}, CSV.read("data/gandk/y_val.csv"; allowmissing=:auto))'

X_training = reshape(X_training, (100,10*100000))
y_training = reshape(y_training, (4,10*100000))

X_test = reshape(X_test, (100,2*100000))
