...
W_h1 = Matrix(CSV.read("data/gandk/DNN for gandk model/W_h1.csv", allowmissing=:auto))
W_h2 = Matrix(CSV.read("data/gandk/DNN for gandk model/W_h2.csv", allowmissing=:auto))
W_h3 = Matrix(CSV.read("data/gandk/DNN for gandk model/W_h3.csv", allowmissing=:auto))
W_out = Matrix(CSV.read("data/gandk/DNN for gandk model/W_out.csv", allowmissing=:auto))

b1 = Matrix(CSV.read("data/gandk/DNN for gandk model/b1.csv", allowmissing=:auto))
b2 = Matrix(CSV.read("data/gandk/DNN for gandk model/b2.csv", allowmissing=:auto))
b3 = Matrix(CSV.read("data/gandk/DNN for gandk model/b3.csv", allowmissing=:auto))
b_out = Matrix(CSV.read("data/gandk/DNN for gandk model/b_out.csv", allowmissing=:auto))
...
calc_summary(y_sim::Vector, y_obs::Vector) = forward_pass(y_sim)[:]
...
CSV.write("data/gandk/simple_dnn_abcrs_post.csv", DataFrame(approx_posterior_samples))
