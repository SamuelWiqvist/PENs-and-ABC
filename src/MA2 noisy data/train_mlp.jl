...
# network = "mlp" # mlp or fully_conncted
...
network_size = ARGS[6] # nbr epochs
...
# generate data
generate_training_test_data = true
...
if generate_training_test_data
    ...
    CSV.write("data/MA2 noisy data/X_training.csv", DataFrame(X_training))
    CSV.write("data/MA2 noisy data/X_val.csv", DataFrame(X_val))
    CSV.write("data/MA2 noisy data/X_test.csv", DataFrame(X_test))

    CSV.write("data/MA2 noisy data/y_training.csv", DataFrame(y_training))
    CSV.write("data/MA2 noisy data/y_val.csv", DataFrame(y_val))
    CSV.write("data/MA2 noisy data/y_test.csv", DataFrame(y_test))

else

    if Knet.gpuCount() > 0

        X_training = Matrix(CSV.read("/lunarc/nobackup/users/samwiq/abc-dl/data/MA2 noisy data/X_training.csv"; allowmissing=:auto))
        X_val = Matrix(CSV.read("/lunarc/nobackup/users/samwiq/abc-dl/data/MA2 noisy data/X_val.csv"; allowmissing=:auto))
        X_test = Matrix(CSV.read("/lunarc/nobackup/users/samwiq/abc-dl/data/MA2 noisy data/X_test.csv"; allowmissing=:auto))

        y_training = Matrix(CSV.read("/lunarc/nobackup/users/samwiq/abc-dl/data/MA2 noisy data/y_training.csv"; allowmissing=:auto))
        y_val = Matrix(CSV.read("/lunarc/nobackup/users/samwiq/abc-dl/data/MA2 noisy data/y_val.csv"; allowmissing=:auto))
        y_test = Matrix(CSV.read("/lunarc/nobackup/users/samwiq/abc-dl/data/MA2 noisy data/y_test.csv"; allowmissing=:auto))


    else

        X_training = Matrix(CSV.read("data/MA2 noisy data/X_training.csv"; allowmissing=:auto))
        X_val = Matrix(CSV.read("data/MA2 noisy data/X_val.csv"; allowmissing=:auto))
        X_test = Matrix(CSV.read("data/MA2 noisy data/X_test.csv"; allowmissing=:auto))

        y_training = Matrix(CSV.read("data/MA2 noisy data/y_training.csv"; allowmissing=:auto))
        y_val = Matrix(CSV.read("data/MA2 noisy data/y_val.csv"; allowmissing=:auto))
        y_test = Matrix(CSV.read("data/MA2 noisy data/y_test.csv"; allowmissing=:auto))
...
if network_size == "large"
    ...
    network_size_string = "large"

else
    # Network Parameters
    n_hidden_1 = 60 # 1st layer number of neurons
    n_hidden_2 = 60 # 2nd layer number of neurons
    n_hidden_3 = 25 # 3nd layer number of neurons
    n_out = nbr_outputs # dim output
    n_input = nbr_features # dim input
    network_size_string = "small"
...
function training(epoch, batch_size)
    ...
    for i = 1:epoch
        ...
        loss_train = loss(w,X_temp, y_temp)/5000
        # calc loss
...
job = network*"_"*string(data_size)*"_"*network_size_string
...
if write_to_files == 1
    ...
    if Knet.gpuCount() > 0
        ...
        CSV.write("/lunarc/nobackup/users/samwiq/abc-dl/data/MA2 noisy data/loss_vec_training_"*job*".csv", DataFrame(loss_vec_training))
        CSV.write("/lunarc/nobackup/users/samwiq/abc-dl/data/MA2 noisy data/loss_vec_val_"*job*".csv", DataFrame(loss_vec_val))
        CSV.write("/lunarc/nobackup/users/samwiq/abc-dl/data/MA2 noisy data/predictions_"*job*".csv", DataFrame(y_pred'))


    else

        CSV.write("data/MA2 noisy data/loss_vec_training_"*job*".csv", DataFrame(loss_vec_training))
        CSV.write("data/MA2 noisy data/loss_vec_val_"*job*".csv", DataFrame(loss_vec_val))
        CSV.write("data/MA2 noisy data/predictions_"*job*".csv", DataFrame(y_pred'))
...
if Knet.gpuCount() > 0
    ...
    proposalas = Matrix(CSV.read("/lunarc/nobackup/users/samwiq/abc-dl/data/MA2 noisy data/abc_data_parameters.csv", allowmissing=:auto))
    datasets = Matrix(CSV.read("/lunarc/nobackup/users/samwiq/abc-dl/data/MA2 noisy data/abc_data_data.csv", allowmissing=:auto))
...
if write_to_files == 1
    ...
    if Knet.gpuCount() > 0
        ...
        CSV.write("/lunarc/nobackup/users/samwiq/abc-dl/data/MA2 noisy data/"*job*"_abcrs_post.csv", DataFrame(approx_posterior_samples))

    else

        CSV.write("data/MA2 noisy data/"*job*"_abcrs_post.csv", DataFrame(approx_posterior_samples))
