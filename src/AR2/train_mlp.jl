...
network_size = parse(Int, ARGS[6]) # nbr epochs
...
if generate_training_test_data
    ...
    CSV.write("data/AR2/X_training.csv", DataFrame(X_training))
    CSV.write("data/AR2/X_val.csv", DataFrame(X_val))
    CSV.write("data/AR2/X_test.csv", DataFrame(X_test))

    CSV.write("data/AR2/y_training.csv", DataFrame(y_training))
    CSV.write("data/AR2/y_val.csv", DataFrame(y_val))
    CSV.write("data/AR2/y_test.csv", DataFrame(y_test))

else

    # load training and test data
    X_training = Matrix(CSV.read("data/AR2/X_training.csv"; allowmissing=:auto))
    X_val = Matrix(CSV.read("data/AR2/X_val.csv"; allowmissing=:auto))
    X_test = Matrix(CSV.read("data/AR2/X_test.csv"; allowmissing=:auto))

    y_training = Matrix(CSV.read("data/AR2/y_training.csv"; allowmissing=:auto))
    y_val = Matrix(CSV.read("data/AR2/y_val.csv"; allowmissing=:auto))
...
if network_size == 1
    ...
    n_hidden_1 = 55 # 1st layer number of neurons
    n_hidden_2 = 55 # 2nd layer number of neurons
...
# init weigths
# xavier initialization, biases are initialized as 0
