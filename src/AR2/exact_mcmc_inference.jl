...
function loglik(y,θ_proposal)
  ...
  partial_ll = zeros(length(y))
  partial_ll[1] = log_normalpdf(0, 1, y[1])
  partial_ll[2] = log_normalpdf(θ_proposal[1]*y[1], 1, y[2])

  for i = 3:length(y)
    partial_ll[i] = log_normalpdf(θ_proposal[1]*y[i-1] + θ_proposal[2]*y[i-2], 1, y[i])
  end
...
function mh(y::Vector, nbr_iter::Int, σ_mh::Real, θ_0::Vector, print_interval::Int=500)
  ...
  for i = 2:nbr_iter
    ...
    θ_proposal = chain[:,i-1] + randn(length(θ_0))*σ_mh
...
CSV.write("data/AR2/exact_mcmc_post.csv", DataFrame(chain[:,burn_in:end]'))
...
CSV.write("data/AR2/posteriors_multiple_data_sets_exact.csv", DataFrame(posteriors'))
